...
function Org4Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/org4.example.com/peers/peer0.org4.example.com/
  fi
...
function addOrg4 () {
  ...
  ${CONTAINER_CLI} exec cli ./scripts/org4-scripts/joinChannel.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
