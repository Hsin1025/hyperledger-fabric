...
function createCCPackage() {
  ...
  peer lifecycle chaincode package basic.tar.gz --path ../asset-transfer-basic/chaincode-go/chaincode --lang golang --label basic_1.0
  if [[ $res -ne 0 ]]; then
    fatalln "Failed to generate package..."
  fi
}

function installCCOnPeer() {
  # install
  infoln "Installing chaincode package on peers"
  export PATH=${PWD}/../bin:$PATH
  export FABRIC_CFG_PATH=$PWD/../config/
  ...
  if [[ $res -ne 0 ]]; then
    fatalln "Failed to install package on org$1"
  fi

  # approve
  infoln "Approving chaincode package on peers"
  QUERY=$(peer lifecycle chaincode queryinstalled)
  PACKAGE_ID=${QUERY:42:74}
  infoln "$PACKAGE_ID"
  ...
  infoln "Successfully install and approve basic$1.tar.gz on org$1"
}
...
function transferAsset() {
  ...
  FUNCTION="{"function":"TransferAsset","Args":["asset$1", "org$2admin"]}"
  infoln "$FUNCTION"
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" -C mychannel -n basic --peerAddresses localhost:7051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt" -c "{\"function\":\"TransferAsset\",\"Args\":[\"asset$1\", \"org$2admin\"]}"
}

function getCallerName() {
  setAdmin $1 $2
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" -C mychannel -n basic --peerAddresses localhost:7051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt" -c '{"function":"GetCallerName","Args":[]}'
}

function readAsset() {
  setAdmin $1 $2
  
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" -C mychannel -n basic --peerAddresses localhost:7051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt" -c "{\"function\":\"ReadAsset\",\"Args\":[\"asset$1\"]}"
}

function getAll() {
  peer chaincode query -C mychannel -n basic$1 -c '{"Args":["GetAllAssets"]}'
}

function getHistoryForKey() {
  setAdmin $1 $2
  
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" -C mychannel -n basic --peerAddresses localhost:7051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt" -c "{\"function\":\"GetHistoryForKey\",\"Args\":[\"asset$1\"]}"
}
...
# peer port default to org1
PEER_PORT=7051
# org default to 1
ORG=1
# asset default to asset1
ASSET=1
...
# default database
DATABASE="leveldb"
# chaincode package name default to basic
NAME="basic"
...
if [[ $# -ge 1 ]] ; then
  ...
  if [[ "$key" == "installCCOnPeer" ]]; then
      export MODE="installCCOnPeer"
      shift
  fi
  if [[ "$key" == "transferAsset" ]]; then
      export MODE="transferAsset"
      shift
  fi
  if [[ "$key" == "readAsset" ]]; then 
      export MODE="readAsset"
      shift
  fi
  if [[ "$key" == "createCCPackage" ]]; then
      export MODE="createCCPackage"
      shift
  fi
  if [[ "$key" == "commitCC" ]]; then
      export MODE="commitCC"
      shift
  fi
  if [[ "$key" == "initCC" ]]; then
      export MODE="initCC"
...
if [ "$MODE" == "up" ]; then
  ...
  deployCCAAS
elif [ "$MODE" == "createCCPackage" ]; then 
  infoln "creating chaincode package on channel '${CHANNEL_NAME}'"
  createCCPackage $ORG
elif [ "$MODE" == "installCCOnPeer" ]; then
  infoln "installing chaincode on peer"
  installCCOnPeer $ORG $PEER_PORT
elif [ "$MODE" == "approveCC" ]; then
  approveCC
elif [ "$MODE" == "commitCC" ]; then
  commitCC $ORG $PEER_PORT
elif [ "$MODE" == "initCC" ]; then
  initCC $ORG $PEER_PORT
elif [ "$MODE" == "transferAsset" ]; then
  transferAsset $ASSET $ORG
elif [ "$MODE" == "getAll" ]; then 
  getAll $ORG $PEER_PORT
  ...
  getHistoryForKey $ORG $PEER_PORT
else
  printHelp
  exit 1
fi
